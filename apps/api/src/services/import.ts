...
export interface HttpFetcher {
  fetch(
    url: string,
    init?: RequestInit,
  ): Promise<Result<{ status: number; text: string; headers: Record<string, string> }, ImportError>>;
}
...
function parseSchemaOrgRecipe(
  ...
  const prepTime =
    typeof obj["prepTime"] === "string" ? obj["prepTime"] : null;
  const cookTime =
    typeof obj["cookTime"] === "string" ? obj["cookTime"] : null;
  const totalTime =
    typeof obj["totalTime"] === "string" ? obj["totalTime"] : null;
  const recipeYield =
    typeof obj["recipeYield"] === "string" ? obj["recipeYield"] : null;

  let image: string | readonly string[] | null = null;
  if (typeof obj["image"] === "string") {
    image = obj["image"];
  } else if (Array.isArray(obj["image"])) {
    image = obj["image"].filter(
      (i): i is string => typeof i === "string",
    );
  }
...
export function createImportService(deps: ImportServiceDeps) {
  ...
  async function processImportJob(
    ...
    const processingPromise = (async (): Promise<
      Result<void, ImportServiceError>
    > => {
      const sourceType = job.source_type;

      switch (sourceType) {
        case "FromUrl":
          return processUrlImport(
            jobId,
            job.source_data as Record<string, unknown>,
          );
    ...
    const timeoutPromise = new Promise<Result<void, ImportServiceError>>(
      (resolve) => {
        setTimeout(() => {
          timedOut = true;
  ...
  async function confirmImport(
    ...
    const title =
      typeof extractData["title"] === "string"
        ? extractData["title"]
        : "Untitled Recipe";
    ...
    const description =
      typeof extractData["description"] === "string"
        ? extractData["description"]
        : null;
    const notes =
      typeof extractData["notes"] === "string"
        ? extractData["notes"]
        : null;
    ...
    const yieldQuantity =
      yieldData !== undefined && typeof yieldData["quantity"] === "number"
        ...
        : null;
    const yieldUnit =
      yieldData !== undefined && typeof yieldData["unit"] === "string"
        ? yieldData["unit"]
        : null;
    ...
    try {
      ...
      if (Array.isArray(ingredientGroups)) {
        for (let groupIdx = 0; groupIdx < ingredientGroups.length; groupIdx++) {
          ...
          const groupLabel =
            typeof group["label"] === "string" ? group["label"] : null;
              ...
              await deps.db.insert(schema.ingredients).values({
                ...
                item:
                  typeof ing["item"] === "string" && ing["item"].length > 0
                    ...
                    : typeof ing["raw_text"] === "string"
                      ...
                      : "Unknown ingredient",
                unit:
                  typeof ing["unit"] === "string" ? ing["unit"] : null,
                quantity_type:
                  ing["quantity"] !== null &&
                ...
                quantity_data:
                  ing["quantity"] !== null && ing["quantity"] !== undefined
                    ...
                    : null,
                notes:
                  typeof ing["notes"] === "string" ? ing["notes"] : null,
                sort_order: ingIdx,
              });
            }
          }
      ...
      if (Array.isArray(extractInstructions) && extractInstructions.length > 0) {
        ...
        if (instrGroupRow !== undefined) {
          for (
            let instrIdx = 0;
            instrIdx < extractInstructions.length;
            instrIdx++
          ) {
  ...
  async function testWordPressConnection(
    ...
    const connectionResult = await deps.wordpress.testConnection(
      siteUrl,
      apiKey,
    );
  ...
  async function syncWordPress(
    ...
    Result<
      { created: number; updated: number; flagged_deleted: number },
      ImportServiceError
    >
  > {
    // Fetch all recipes from WordPress
    const recipesResult = await deps.wordpress.fetchRecipes(
      siteUrl,
      apiKey,
      plugin,
    );
    ...
    const wpRecipeIds = new Set(
      wpRecipes.map((r) => r.wordpress_recipe_id),
    );
...
export function createDefaultWordPressClient(
  fetcher: HttpFetcher,
): WordPressClient {
  ...
  return {
    async testConnection(
      ...
      try {
        const data = JSON.parse(result.value.text) as Record<
          string,
          unknown
        >;
    ...
    async detectPlugin(
      ...
      // Try WP Recipe Maker
      const wprmResult = await fetcher.fetch(
      ...
      // Try Tasty Recipes
      const tastyResult = await fetcher.fetch(
        ...
        for (const item of data) {
          ...
          recipes.push({
            ...
            instructions: Array.isArray(obj["instructions"])
              ? (obj["instructions"] as unknown[]).filter(
                  (i): i is string => typeof i === "string",
                )
              : [],
            prep_minutes:
              typeof obj["prep_minutes"] === "number"
                ? obj["prep_minutes"]
                : null,
            cook_minutes:
              typeof obj["cook_minutes"] === "number"
                ? obj["cook_minutes"]
                : null,
            total_minutes:
              typeof obj["total_minutes"] === "number"
                ? obj["total_minutes"]
                : null,
            yield_quantity:
              typeof obj["yield_quantity"] === "number"
                ? obj["yield_quantity"]
                : null,
            yield_unit:
              typeof obj["yield_unit"] === "string"
                ? obj["yield_unit"]
                : null,
            modified:
