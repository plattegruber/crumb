<script lang="ts">
	...
	let serverSignedIn = $derived(data.userId !== null);
	// Combine server-side and client-side auth state
	let signedIn = $derived(serverSignedIn || clerkSignedIn);
	...
	let isPublicPage = $derived(currentPath === '/' || isAuthPage);
	let showApp = $derived(signedIn || isPublicPage || !clerkLoaded);
