...
<Aside>
  A recipe with no score is different from a recipe with a score of 0. No score means the recipe has not been included in any emails yet. A score of 0 would mean it was sent but received zero engagement (which is extremely rare).
</Aside>
...
<Aside type="tip">
  Revenue attribution only works for products sold through platforms that report purchases back to Kit (via Kit Commerce or purchase webhooks). If you sell through a platform without Kit integration, attribution will not be tracked.
</Aside>
