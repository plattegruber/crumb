...
<form class="search-bar" onsubmit={handleSubmit}>
	<div class="search-row">
		...
		>
			Filters
			{#if hasFilters}
				<span class="filter-count">{(localStatus !== '' ? 1 : 0) + (localMealType !== '' ? 1 : 0) + localDietaryTags.length}</span>
			{/if}
		</button>
	</div>
		...
		<div class="filters">
			...
			<div class="filter-group full-width">
				...
				<div class="tag-buttons" role="group" aria-labelledby="dietary-label">
					{#each dietaryTagEntries as [, value]}
						<button
							type="button"
